...
;in its name to 0. Note that it does not guarantee there will be a blank tile left behind when removed.
	...
	!IRQ_line_Y_pos = $52		;\($26 by default), layer 3 status bar/HUD IRQ/cutoff line Y position (in pixels),
					;/If you are not using the last row of 8x8 tiles, change it to $24.

	!Enable_TIME	= 0		;>display time limit: 0 = false (will be black squares on 1st and 2nd digits,
	...
	!timer_warning	= 0		;>if the timer hits below 100, the "TIME" and digits palette changes to red and if
					...
					; 0 = false, 1 = true.
	!Enable_SCORE	= 0		;>Display score: 0 = false (will display last 0 digit, also you need to download GHB's
	...
	!COINS_OFFSET	= $0F8		;>Position of coin counter

	!Enable_NAME	= 0	;>Show Mario/Luigi name: 0 = false (always say "MARIO"), 1 = true.
	!NAME_OFFSET	= $084		;>Position of player name (MARIO/LUIGI)

	!Enable_DRAGON	= 0		;>Display (repeating by # of icons) yoshi coins: 0 = false, 1 = true.
