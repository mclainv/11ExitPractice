...
!offsetXCoord = $02+!offset
!offsetInput = $18+!offset
!offsetClipInfo = $D0+!offset
...
XCoord:
		LDA #$21 ;X
		STA $7FA000+!offsetXCoord+$00
		LDA #$27 ;-
		STA $7FA000+!offsetXCoord+$02
		LDA #$0c ;c
		STA $7FA000+!offsetXCoord+$04
		LDA #$18 ;o
		STA $7FA000+!offsetXCoord+$06
		LDA #$18 ;o
		STA $7FA000+!offsetXCoord+$08
		LDA #$1b ;r
		STA $7FA000+!offsetXCoord+$0A
		LDA #$0d ;d
		STA $7FA000+!offsetXCoord+$0C
		...
		+
		JSR HexToDec
		STA $7FA000+!offsetXCoord+$42
		STA $7FA000+!offsetXCoord+$40
		TXA
		BNE + ;if x (now A) =/= 0 (10s digit)
		LDA #$FC
		STA $7FA000+!offsetXCoord+$42
		BRA ++
		+
		STA $7FA000+!offsetXCoord+$40 ; Store X-Digit
		++
	; green if correct pixel (defined in $63)
		LDA.b $14 ;
		BMI ++
		CMP #$0C
		BCC ++ ; Branch if less than
		CMP #$10
		BCS ++ ; Branch if greater than or equal to
		LDA #$28
		BRA +
		++
		+++
		...
		LDA #$38
		+
		STA $7FA000+!offsetXCoord+$41
		STA $7FA000+!offsetXCoord+$43
...
-	LDA str_delay,X
	...
	LDX #$10
-	LDA str_duck,X
	STA !RAM_BAR+!offsetClipInfo+$40,X
	DEX
	DEX
	BPL -
	...
	LDA !huddelay
	JSR HexToDec
	STA !RAM_BAR+!offsetClipInfo+$10
	TXA
	BNE ++ ; if 1s val is 0
		LDA #$FC
	++
	STA !RAM_BAR+!offsetClipInfo+$0E
	...
	lda !huddelay
			...
			bcs ++
				lda #$28
				STA !RAM_BAR+!offsetClipInfo+$11
				STA !RAM_BAR+!offsetClipInfo+$0F
			++
	;	duck
	LDA !hudduck
	JSR HexToDec
	STA !RAM_BAR+!offsetClipInfo+$40+$0E
	TXA
	BNE ++ ; if 1s val is 0
		LDA #$FC
	++
	STA !RAM_BAR+!offsetClipInfo+$40+$0C

	lda !hudduck
		cmp #$02
		bne ++
			lda #$28
			STA !RAM_BAR+!offsetClipInfo+$40+$0F
			STA !RAM_BAR+!offsetClipInfo+$40+$0D
		++
	RTS
